...
POST /api/chat  { message, session_id, app_id }
    ...
    ├─ 2. 召回记忆            → stage("召回记忆", active/done)
    │                         → memory({ snippets: [...] })
    ├─ 3. 检索知识            → stage("检索知识", active/done)
    ├─ 4. 构建 Prompt         注入记忆片段 + 知识片段 + 工具描述
    ├─ 5. 调用 LLM（流式）    → think / text / artifact 事件
    ├─ 6. 执行 Tools（如有）  → extension("tool_progress", ...)
    └─ 7. 完成               → stage("生成回复", done) + done({})
```
...
| 处理阶段进度 | `stage` | StageTimeline |
| 记忆召回结果 | `memory` | Memory 芯片 |
| 工具调用进度 | `extension`（name: `tool_progress`） | renderExtension 自定义卡片 |
