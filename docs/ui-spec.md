...
- 展开宽度：`260px`（CSS token：`--session-col-w`），折叠宽度：`28px`
- 过渡动画：`width 0.25s cubic-bezier(.4,0,.2,1)`
- 展开时 header：`[新建][搜索]`（左）+ `[←]`（右）；折叠时 header 隐藏，仅显示 `[→]`
...
| **折叠态** | 仅显示头像圆圈，居中（姓名 `opacity:0; max-width:0`）；展开后显示头像 + 姓名 |
| **菜单弹出位置** | `position:absolute; bottom:calc(100% + 4px)` — 从用户行向上弹出，不超出视口 |

禁止：把账号菜单放到顶栏、用 modal 弹窗展示账号信息、把设置入口独立到其他地方。
...
| 规则 | 说明 |
|------|------|
| **展开态（260px）** | 顶部 header 行显示 `[新建] [搜索]`（左侧）+ `[←]`（右侧）；下方为会话列表 |
| **折叠态（28px）** | header 隐藏；显示一条 28px 宽的竖条，其中仅有 `[→]` 按钮居顶 |
| **触发折叠** | header 右侧 `←` 箭头按钮 |
| **触发展开** | 折叠条中的 `→` 箭头按钮 |
| **过渡** | `width 0.25s cubic-bezier(.4,0,.2,1)` |
...
禁止：hover 自动折叠/展开、点击会话列表区折叠、外部按钮控制（应内嵌到列本身）。

### 8.5c Artifact 面板开关

| 规则 | 说明 |
|------|------|
| **自动触发** | 后端推送 `artifact` SSE 事件时，系统自动展开 Artifact 面板 |
| **手动触发** | 顶栏（topbar）右侧的 Artifact 图标按钮，用户可随时切换显示/隐藏 |
| **按钮位置** | `layout-topbar` 最右侧，`topbar-spacer` 后面 |
| **按钮激活态** | 面板可见时按钮高亮（`active` class） |
| **动画** | `width + opacity, 0.25s cubic-bezier(.4,0,.2,1)` |

禁止：把 Artifact 开关放到外部控制栏、仅允许系统自动触发（用户必须能手动控制）。
...
| 会话栏折叠/展开 | `0.25s cubic-bezier(.4,0,.2,1)` |
